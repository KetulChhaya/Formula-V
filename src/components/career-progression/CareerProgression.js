...
const CareerProgression = () => {
  ...
  useEffect(() => {
    loadData().then(
      ({ drivers, driver_standings, results, races, constructors }) => {
        // restrict to seasons 1995–2024
        const filteredRaces = races.filter(
          (r) => +r.year >= 1995 && +r.year <= 2024
        );
        const raceMap = Object.fromEntries(
          filteredRaces.map((r) => [r.raceId, r])
        );
        ...
        const consMap = Object.fromEntries(
          constructors.map((c) => [
            c.constructorId,
            { name: c.name, ref: c.constructorRef },
          ])
        );
        ...
        ids.forEach((driverId) => {
          for (let year = 1995; year <= 2024; year++) {
            ...
            // final points
            const stRows = (standingsByDY[key] || []).sort(
              (a, b) => +raceMap[a.raceId].round - +raceMap[b.raceId].round
            );
        ...
        const h = Object.entries(dataMap).find(
          ([, info]) => info.name === "Lewis Hamilton"
        );
  ...
  const sortedDrivers = driverData
    ? Object.entries(driverData.drivers)
        .map(([id, info]) => {
          const first =
            info.yearsData.find((d) => d.constructorRef)?.year || Infinity;
      ...
      <div style={{ display: "flex", alignItems: "flex-start" }}>
        ...
        <div className="flex flex-col" style={{ maxWidth: "1100px" }}>
          ...
          <h3 className="font-[Formula1] mt-5 justify-center text-center">
            Explore how a driver’s career unfolded season by season. This chart
            tracks their total points each year, letting you see breakout
            seasons, steady growth, or sudden declines
          </h3>
        </div>
      </div>
    </div>

...
const ChampionshipBattleTrends = () => {
    ...
    useEffect(() => {
        if (!races.length || !drivers.length || !driverStandings.length || !selectedSeason) return;
    ...
    useEffect(() => {
        ...
        g.select('.labels-end')
            .selectAll('.label-end')
            .data(seasonData.drivers.filter(d => d.positions[d.positions.length - 1] !== null)) // Only label drivers with a position in the last round
            .join('text')

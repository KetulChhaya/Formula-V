...
const TrackPerformance = () => {
  ...
  useEffect(() => {
    loadData().then(({ drivers, results, races, circuits, constructors }) => {
      ...
      const raceMap     = Object.fromEntries(races.map(r => [r.raceId, r.circuitId]));
      const circuitMap  = Object.fromEntries(circuits.map(c => [c.circuitId, c.name]));
      const driverMap   = Object.fromEntries(drivers.map(d => [d.driverId, `${d.forename} ${d.surname}`]));

      refs.current = { raceMap, validRaceIds, circuitMap, fullResults: results, driverMap };
      ...
      const ids = Object.keys(td).sort((a,b) => td[a].name.localeCompare(td[b].name));
      setTrackData(td);
      setTrackIds(ids);
  ...
  useEffect(() => {
    ...
    // scales & generators
    const color = d3.scaleOrdinal(d3.schemeCategory10).domain(data.map(d=>d.driverName));
